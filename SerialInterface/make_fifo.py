...
def make_fifo():
    ...
    FIFO_ALMOSTFULL_THRESHOLD = m.Parameter('FIFO_ALMOSTFULL_THRESHOLD', EmbeddedNumeric('2**FIFO_DEPTH_BITS - 2'))
    ...
    initial = m.Initial()
    initial.add(empty(Int(1, 1, 2)))
    initial.add(almostempty(Int(1, 1, 2)))
    initial.add(full(Int(0, 1, 2)))
    initial.add(almostfull(Int(0, 1, 2)))
    initial.add(rp(0))
    initial.add(wp(0))
    initial.add(count(0))
